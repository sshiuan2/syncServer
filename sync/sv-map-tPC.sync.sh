...
sed -i "s/^thisServerPort=.*$/thisServerPort=25568/g" $varPath
...
sed -i 's/^difficulty=.*$/difficulty=0/g' $to
...
local url="http://up9cloud.net/download/resource-pack/tPC_Sound_Pack_v2.1.1.zip"
sed -i "s/^resource-pack=.*$/resource-pack=$url/g" $to
}
sync_start(){
